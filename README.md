...
1. **多城市数据采集**：内置 29 个城市，链家/贝壳双平台爬虫；真实请求失败时自动回退到内置模拟数据，保证离线可运行。
...
当前数据库为真实数据，不含任何模拟或合成房源。

| 城市 | 房源明细 | 覆盖年份 | 来源 |
|------|----------|----------|------|
| 北京 | 318,851 条 | 2010–2018（趋势拟合区间） | 链家真实成交数据集 |
| 上海 | 93,055 条 | 2022 | 链家真实房源数据集 |

另有 `city_index` 表存放国家统计局 70 城房价指数 47,460 行，覆盖 70 城、
2006–2026 年新房与二手房的同比 / 环比 / 定基比指数。

**历年走势的两种口径**（接口通过 `data_source` 字段标明）：

- `真实成交`：房源覆盖多个年份时，直接按年聚合真实成交均价（如北京）。
- `官方指数折算`：房源仅覆盖单一年份时（如上海仅 2022 年），以该年真实成交
  均价为锚点，按统计局二手住宅同比指数链式折算历年价格水平。统计局指数是
  质量调整后的同质可比价格，链式累乘年数过多会低估名义涨幅，故折算范围
  限制在最近 10 年内。
